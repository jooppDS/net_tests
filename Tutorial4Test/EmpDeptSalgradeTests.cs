...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = (from e in emps where (from d in depts where d.Loc.Equals("CHICAGO") select d.DeptNo).Contains(e.DeptNo) select e).ToList(); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(emp => emp.Comm.HasValue).Select(emp => new
        { 
            emp.EName,
           emp.Comm
        });
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = (from emp in emps join g in grades on true equals true where emp.Sal >= g.Losal && emp.Sal <= g.Hisal select new
        {
            emp.EName,
            g.Grade
        });
